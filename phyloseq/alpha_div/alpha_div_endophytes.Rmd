...
SeqDepth
ggsave("./Figures_endophytes/SeqDepth_plot.png", plot = SeqDepth, width = 6, height = 4, units = "in")
```
...
```{r warning=FALSE}
set.seed(1)
physeq.prune.rarefy = rarefy_even_depth(physeq.prune, sample.size = 5202, replace = FALSE, trimOTUs = FALSE)
physeq.prune.rarefy
```
...
Save ggplot as pdf to Figures_endophytes folder using the full path

###STEP11.1: Plot Fungal Alpha diversity by Crust_type (Chao1 and Observed) FOR ENDOPHYTES
...
MAKE THIS FOR ENDOPHYTES ONLY:
```{r}
physeq.prune.rarefy = subset_samples(physeq.prune.rarefy, Crust_type=="SMC.ster")
```
...
```{r}
pdf("./Figures_endophytes/Alpha_Diversity_(Chao1)_by_Crust_type.pdf", width = 8, height = 5)
physeq.prune.rarefy.plot.richness.Crust_type
dev.off()
```
...
```{r}
pdf("./Figures_endophytes/Alpha_Diversity_(Observed)_Crust_type.pdf", width = 8, height = 5)
physeq.prune.rarefy.plot.richness.Crust_type
dev.off()
```
...
```{r warning=FALSE}
pdf("./Figures_endophytes/Alpha_Div_Site_(Chao1).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Site
dev.off()
```
...
```{r warning=FALSE}
pdf("./Figures_endophytes/Fungal_Alpha_Div_by_Site_(Observed).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Site
dev.off()
```
...
```{r warning=FALSE}
pdf("./Figures_endophytes/Alpha_Div_by_Climate_(Chao1).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.SF.Coastal
dev.off()
```
...
```{r warning=FALSE}
pdf("./Figures_endophytes/Alpha_Div_by_plant_proximity_(Chao1).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.SF.near_plant
dev.off()
```
...
```{r}
pdf("./Figures_endophytes/Fungal_Class_Alpha_Div_by_Site.pdf", width = 8, height = 5)
...
###STEP11.5: Plotting alpha diversity for each class by Climate

```{r}
pdf("./Figures_endophytes/Fungal_Class_Alpha_Div_by_Climate.pdf", width = 8, height = 5)

for (i in FungiClass) {
  APS = subset_taxa(physeq.prune.rarefy.SF, Class == i)
  APS.plot = plot_richness(APS, x="Climate", color =("Climate"), measures=c("Observed", "Chao1")) +
    ggtitle(i) +
    stat_compare_means(method = "anova") +
    theme_bw() +
    scale_color_manual(values = high_contrast_colors) +  
...
###STEP11.6: Plotting alpha diversity for each class by Plant proximity

```{r}
pdf("./Figures_endophytes/Fungal_Class_Alpha_Div_by_Plant_proximity.pdf", width = 8, height = 5)

for (i in FungiClass) {
  APS = subset_taxa(physeq.prune.rarefy.SF, Class == i)
  APS.plot = plot_richness(APS, x="Near_plant", color =("Near_plant"), measures=c("Observed", "Chao1")) +
    ggtitle(i) +
    stat_compare_means(method = "anova") +
    theme_bw() +
    scale_color_manual(values = high_contrast_colors) +  
