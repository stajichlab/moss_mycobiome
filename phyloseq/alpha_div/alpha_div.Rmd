...
```{r}
meta = read.table("metadata_update2.tsv",header=TRUE,row.names=1,sep="\t",stringsAsFactors=FALSE)
```
...
```{r}
physeq.prune.rarefy.plot.richness.Substrate = plot_richness(physeq.prune.rarefy, x="Substrate", color=("Substrate"), measures=c("Observed")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Observed) by Crust type") + stat_compare_means(method = "anova", label.y = 3000) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Dirt", label.y = 2800, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 
...
```{r}
physeq.prune.rarefy.SF = subset_samples(physeq.prune.rarefy, Layer=="Surface")
sample_data(physeq.prune.rarefy.SF)$Site = factor(sample_data(physeq.prune.rarefy.SF)$Site, levels = c("CIMA", "ODLO", "AB", "GMT","TP"))
```
...
```{r}
ylabel <- data.frame("ylabel" = c(245,235,215,175,135))
