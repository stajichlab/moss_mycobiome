...
```{r}
pdf("./Figures/Rarefaction_Curve.pdf", width = 8, height = 5 )
ggplot(data = rare)+
  geom_line(aes(x = Sample.size, y = OTU, color = sample), show.legend = FALSE)+
  scale_x_continuous(labels =  waiver()) + theme_bw() + ggtitle("Fungal Rarefaction Curve")+
  theme(plot.title = element_text(hjust = 0.5))
...
```{r}
physeq.prune.rarefy.plot.richness.Substrate = plot_richness(physeq.prune.rarefy, x="Substrate", color=("Substrate"), measures=c("Chao1")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Chao1) by Substrate") + stat_compare_means(method = "anova", label.y = 3000) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Dirt", label.y = 2800, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 

physeq.prune.rarefy.plot.richness.Substrate
```

```{r}
pdf("./Figures/Alpha_Diversity_(Chao1)_by_Substrate.pdf", width = 8, height = 5)
physeq.prune.rarefy.plot.richness.Substrate
dev.off()
```
####Supplemental: by site and substrate

```{r}
physeq.prune.rarefy.plot.richness.Substrate.site = plot_richness(physeq.prune.rarefy, x="Site_Substrate", color=("Site_Substrate"), measures=c("Chao1")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Chao1) by Site and substrate") + stat_compare_means(method = "anova", label.y = 3000) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Dirt", label.y = 2800, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 

physeq.prune.rarefy.plot.richness.Substrate.site
```
####Supplemental: Observed

```{r}
physeq.prune.rarefy.plot.richness.Substrate = plot_richness(physeq.prune.rarefy, x="Substrate", color=("Substrate"), measures=c("Observed")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Observed) by Crust type") + stat_compare_means(method = "anova", label.y = 1500) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Dirt", label.y = 900, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 
...
```{r}
pdf("./Figures/Alpha_Diversity_(Observed)_Substrate.pdf", width = 8, height = 5)
physeq.prune.rarefy.plot.richness.Substrate
dev.off()
```
...
####Kian note: REMOVE DIRT and subsurface samples for bar plot, note that layer for dirt is subsurface. Changed so that the layer was surface for SMC.sub
```{r}
...
```{r warning=FALSE}
pdf("./Figures/Alpha_Div_Site_(Chao1).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Site
dev.off()
```

```{r}
physeq.prune.rarefy.plot.richness.Site = plot_richness(physeq.prune.rarefy.SF, x="Site", color=("Site"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Mojave Biocrust Fungal Alpha Diversity by Site") + stat_compare_means(method = "anova", label.y = 1500) + theme(plot.title = element_text(hjust = 0.5)) + geom_text(data=LABELS, aes(x=LABELS$Site, y = LABELS$ylabel , label = LABELS$Letters)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 
physeq.prune.rarefy.plot.richness.Site
```
...
###STEP11.3.1: Plot Fungal Alpha diversity by Climate

```{r}
physeq.prune.rarefy.plot.richness.SF.Coastal = plot_richness(physeq.prune.rarefy.SF, x="Climate", color=("Climate"), measures=c("Chao1")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Chao1) by Climate") + stat_compare_means(method = "anova", label.y = 3000) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Coastal", label.y = 2800, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 

physeq.prune.rarefy.plot.richness.SF.Coastal
```

```{r warning=FALSE}
pdf("./Figures/Alpha_Div_by_Climate_(Chao1).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.SF.Coastal
dev.off()
```

###STEP11.3.2: Plot Fungal Alpha diversity by Nearness to plants

```{r}
physeq.prune.rarefy.plot.richness.SF.near_plant = plot_richness(physeq.prune.rarefy.SF, x="Near_plant", color=("Near_plant"), measures=c("Chao1")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Chao1) by plant proximity") + stat_compare_means(method = "anova", label.y = 3000) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Near_plant", label.y = 2800, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 

physeq.prune.rarefy.plot.richness.SF.near_plant
```

```{r warning=FALSE}
pdf("./Figures/Alpha_Div_by_plant_proximity_(Chao1).pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.SF.near_plant
dev.off()
```

```{r}
physeq.prune.rarefy.dirt = subset_samples(physeq.prune.rarefy, Substrate=="Dirt")
physeq.prune.rarefy.dirt
```

```{r}
physeq.prune.rarefy.plot.richness.dirt.near_plant = plot_richness(physeq.prune.rarefy.dirt, x="Near_plant", color=("Near_plant"), measures=c("Chao1")) + geom_boxplot() + ggtitle("Dirt Fungal Alpha Diversity (Chao1) by plant proximity") + stat_compare_means(method = "anova", label.y = 3000) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "Near_plant", label.y = 2800, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = Dali) 
...
for (i in FungiClass) {
  ...
  APS.plot = plot_richness(APS, x="Site", color =("Site"), measures=c("Observed", "Chao1")) +
    ...
    theme_bw() +
    scale_color_manual(values = Warhol) +  
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
    geom_boxplot() +
    theme(plot.title = element_text(hjust = 0.5))
  print(APS.plot)
...
```
```{r}
pdf("./Figures/Fungal_Class_Alpha_Div_by_Site_black.pdf", width = 8, height = 5)
color1 <- c("black")
for (i in FungiClass) {
  APS = subset_taxa(physeq.prune.rarefy.SF, Class == i)
  APS.plot = plot_richness(APS, x="Site", color = "color", measures=c("Observed", "Chao1")) +
    ggtitle(i) +
    stat_compare_means(method = "anova") +
    theme_bw() +
...
```{r}
pdf("./Figures/Fungal_Class_Alpha_Div_by_Substrate.pdf", width = 8, height = 5)

for (i in FungiClass) {
  APS = subset_taxa(physeq.prune.rarefy, Class == i)
  APS.plot = plot_richness(APS, x="Substrate", color =("Substrate"), measures=c("Observed", "Chao1")) +
    ggtitle(i) +
    stat_compare_means(method = "anova") +
    theme_bw() +
    scale_color_manual(values = Warhol) +  
...
```{r}
pdf("./Figures/Fungal_Genus_Alpha_Div_by_Substrate.pdf", width = 8, height = 5)
