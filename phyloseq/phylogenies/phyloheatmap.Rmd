...
pgroup <- groupClade(object, c(182, 277, 158))

p3 <- ggtree(pgroup, layout="rectangular", aes(color=group)) + 
  scale_color_manual(values=c("black", "firebrick", "steelblue","pink")) + geom_tiplab(size = .8, align = TRUE, linesize = .25, offset = 1, linetype = 3, color = "grey") 

#plot a test tree to find nodes for labeling
ptest <- ggtree(object, layout="rectangular",size=1,aes(color=as.numeric(label))) +
...
p_heatmap <- gheatmap(p3, log_df_otu, width=.75, offset=0.1, 
