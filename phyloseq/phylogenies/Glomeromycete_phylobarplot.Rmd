...
p1 <- ggtree(tree, layout="rectangular") %<+% new_tip_labels + geom_tippoint(aes(color=Taxonomy), size=2) +
...
ptest
```
#### Save the test treeand check nodes for labeling


```{r}

# Save the test and check nodes for labeling
ggsave("test.png", plot = ptest, width = 17, height = 9)
...
p2 <- facet_plot(p1, panel = 'ASV Relative Abundance by Substrate', data = Crust_type.df.ratio, 
				geom = geom_barh, 
				mapping = aes(x = ratio, fill = Crust_type), 
				stat='identity' ) +
  scale_fill_manual(values = OKeeffe3)
p2
...
p3 <- facet_plot(p2, panel = 'ASV Relative Abundance by Site', data = Site.df.ratio, 
				geom = geom_barh, 
				mapping = aes(x = ratio, fill = Site), 
				stat='identity' ) + guides(fill=guide_legend(title="Barplot Legend")) + 
  scale_fill_manual(values = OKeeffe3, breaks = c("AB", "CIMA", "GMT", "ODLO", "TP", "Dirt", "SMC","SMC.ster","SMC.sub"), labels = c("Anza Borrego", "CIMA Volcanic Field", "Gramite Mountains", "Oasis De Los Osos", "Torrey Pines", "Dirt", "Moss Crust","Sterile Moss","Subsurface")) +
    scale_x_ggtree()
p3
...
ggsave("glomeromycetebarplot_final.png", plot = p3, width = 17, height = 9)
...
ptest <- facet_plot(p1, panel = 'ASV Relative Abundance by Substrate', data = Crust_type.df.ratio, 
				geom = geom_barh, 
				mapping = aes(x = Count, fill = Crust_type), 
				stat='identity' ) +
  scale_fill_manual(values = OKeeffe3)
ptest
...
ptest2 <- facet_plot(ptest, panel = 'ASV Relative Abundance by Site', data = Site.df.ratio, 
				geom = geom_barh, 
