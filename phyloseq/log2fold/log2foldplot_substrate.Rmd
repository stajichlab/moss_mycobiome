...
otu_tab <- otu_table(physeq.prune.Substrate)
...
physeq.prune.Substrate <- merge_phyloseq(physeq.prune.Substrate, otu_tab_modified)
...
DEseq_substrate <- phyloseq_to_deseq2(physeq.prune.Substrate, ~ Substrate)
...
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(physeq.prune.Substrate)[rownames(sigtab), ], "matrix"))
head(sigtab)
```
...
log2plot <- ggplot(sigtab_filtered, aes(x=Genus, y=log2FoldChange, color=Class)) + 
  ...
  ggtitle("Post vs pre surface sterilization moss crust mycobiome") + 
...
ggsave("log2plot_before_and_after_surface_sterilization.png", plot = log2plot, width = 8, height = 6)
