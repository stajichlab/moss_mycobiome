...
physeq.prune = subset_samples(physeq.prune, Layer=="Surface")
...
physeq.prune.Climate <- subset_samples(physeq.prune, Climate %in% deserts)

physeq.prune.Climate
```
...
otu_tab <- otu_table(physeq.prune.Climate)
...
physeq.prune.Climate <- merge_phyloseq(physeq.prune.Climate, otu_tab_modified)
...
DEseq_Climate <- phyloseq_to_deseq2(physeq.prune.Climate, ~ Climate)
...
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(physeq.prune.Climate)[rownames(sigtab), ], "matrix"))
head(sigtab)
```
