...
```{r}
meta = read.table("metadata_update3.tsv",header=TRUE,row.names=1,sep="\t",stringsAsFactors=FALSE)
```
...
Subset so that only the variables of interest are present
```{r}
deserts <- c("Mojave_Desert", "Colorado_Desert")

# Subset the samples based on the crust types
physeq.prune.rarefy.Climate <- subset_samples(physeq.prune.rarefy, Climate %in% deserts)

physeq.prune.rarefy.Climate
```
...
otu_tab <- otu_table(physeq.prune.rarefy.Climate)
...
physeq.prune.rarefy.Climate <- merge_phyloseq(physeq.prune.rarefy.Climate, otu_tab_modified)
...
DEseq_Climate <- phyloseq_to_deseq2(physeq.prune.rarefy.Climate, ~ Climate)
DEseq_Climate <- DESeq(DEseq_Climate, test="Wald", fitType="parametric")
...
```{r}
res = results(DEseq_Climate, cooksCutoff = FALSE)
alpha = 0.01
sigtab = res[which(res$padj < alpha), ]
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(physeq.prune.rarefy.Climate)[rownames(sigtab), ], "matrix"))
head(sigtab)
```
...
log2plot <- ggplot(sigtab_filtered, aes(x=Genus, y=log2FoldChange, color=Class)) + 
  ...
  ggtitle("Mojave vs Colorado Desert mycobiome log2Fold change") + 
...
ggsave("log2plot_Mojave_vs_CO.png", plot = log2plot, width = 8, height = 6)
