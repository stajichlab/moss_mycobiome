...
# Perform ordination
physeq.prune.x.SF.ord <- ordinate(physeq.prune.x, "PCoA", "bray")
...
Warhol <- friendly_pal("ito_seven")

```


```{r}
pscrusttype = plot_ordination(physeq.prune.x, physeq.prune.x.SF.ord, type = "Substrate", color = "Substrate")  + theme_bw() + stat_ellipse(geom = "polygon", alpha = 1/12) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + geom_point(size=5) + scale_colour_manual(values = Warhol) + annotate("text", x = 1, y = 0.54, label = "PERMANOVA, p = 0.006")

pscrusttype

```
```{r}
png("./Figures_glom/Fungal Beta Diversity (PCoA) by Substrate.png", units="in", width = 8, height = 5, res = 300 )
pscrusttype
dev.off()
```
###Beta by Crust site
...
```{r}
pssite = plot_ordination(physeq.prune.SF.x, physeq.prune.x.SF.ord.2, type = "Site", color = "Site")  + theme_bw() + stat_ellipse(geom = "polygon", alpha = 1/12) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + geom_point(size=5) + scale_colour_manual(values = Warhol) + annotate("text", x = -.5, y = -.7, label = "PERMANOVA, p = 0.003")
...
```{r}
library('patchwork')
combo <- pssite + pscrusttype + plot_annotation(tag_levels = 'A')
```

```{r}
svg("./site_and_substrate_combined.svg", width = 14, height = 7)
combo
dev.off()
```

```{r}
png("./Figures_glom/Fungal Beta Diversity (PCoA) by site.png", units="in", width = 8, height = 5, res = 300 )
pssite
dev.off()
```
...
```{r}
psClimate = plot_ordination(physeq.prune.SF.x, physeq.prune.x.SF.ord.2, type = "Climate", color = "Climate")  + theme_bw() + stat_ellipse(geom = "polygon", alpha = 1/12) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + geom_point(size=5) + scale_colour_manual(values = Warhol) 

psClimate
```

```{r}
pdf("./Figures_glom/Fungal Beta Diversity (PCoA) by Climate.pdf")
psClimate
dev.off()
```
...
```{r}
pdf("./Figures_glom/Fungal Beta Diversity (PCoA) by Plant Proximity.pdf")
psPlant
dev.off()
```
