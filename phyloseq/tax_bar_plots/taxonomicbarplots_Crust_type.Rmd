...
```{r warning=FALSE}
set.seed(1)
physeq.prune.rarefy = rarefy_even_depth(physeq.prune, sample.size = 5208, replace = FALSE, trimOTUs = FALSE)
physeq.prune.rarefy
```
...
```{r}
data_glom.fun$Class <- factor(data_glom.fun$Class, levels = c("Agaricomycetes", "Sordariomycetes", "Microbotryomycetes", "Dothideomycetes", "Eurotiomycetes", "Unknown", "Saccharomycetes","Pezizomycetes", "Rhizophlyctidomycetes", "Tremellomycetes", "Glomeromycetes","Leotiomycetes","Mortierellomycetes","<1% abund."))
```
...
```{r}
data_glom.fun$Crust_type = factor(data_glom.fun$Crust_type, levels = c("Moss Crust","Subsurface", "Dirt", "Surface Sterile"))
```
```{r}
data_glom.fun$Site = factor(data_glom.fun$Site, levels = c("AB", "CIMA", "ODLO", "TP","GMT"))
```

```{r fig.height=5, fig.width=8, fig.align="center"}
# The palette with black:
OKeeffe <- c("#796B58", "#1C1A18", "#95D1E6","#D2BAA4", "#96B5BC")
```
...
```{r}
png("./Figures/Fungal_Taxonomic_Composition_(Class_level)_by_Crust_type.png", units="in", width = 5.8, height = 5.8, res = 300 )
taxcom_layer_pl
dev.off()
```
