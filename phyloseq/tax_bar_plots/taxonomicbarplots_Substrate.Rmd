---
title: "tax bar plots substrate"
author: "Nat Pombubpa, a bit by Kian Kelly"
date: "Updated 04/2024"
...
```{r}
data_glom.fun$Class <- factor(data_glom.fun$Class, levels = c("Agaricomycetes", "Sordariomycetes", "Dothideomycetes", "Microbotryomycetes", "Unknown", "Eurotiomycetes", "Pezizomycetes","Saccharomycetes", "Tremellomycetes", "Rhizophlyctidomycetes", "Mortierellomycetes","Glomeromycetes","Leotiomycetes","Basidiobolomycetes", "Lecanoromycetes", "<1% abund."))
```
...
```{r}
data_glom.fun$Substrate = factor(data_glom.fun$Substrate, levels = c("Moss Crust","Subsurface", "Dirt", "Surface Sterile"))
```
```{r}
data_glom.fun$Site = factor(data_glom.fun$Site, levels = c("AB", "CIMA", "ODLO", "TP","GMT", "TP2"))
```
...
taxcom_layer_pl = ggplot(data = data_glom.fun, mapping = aes_string(x = "Substrate" ,y = "Abundance", fill = "Class" )) + 
...
```{r}
png("./Figures/Fungal_Taxonomic_Composition_(Class_level)_by_Substrate.png", units="in", width = 5.8, height = 5.8, res = 300 )
taxcom_layer_pl
dev.off()
```
