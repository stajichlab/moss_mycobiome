...
#Fungal taxonomic barplot by Near_Plant
...
```{r}
fun.abund <- merge_samples(glom.fun, "Near_plant")
sample_data(fun.abund)$Near_Plant <- factor(sample_names(fun.abund))
...
```{r}
data_glom.fun$Class <- factor(data_glom.fun$Class, levels = c("Agaricomycetes", "Dothideomycetes", "Eurotiomycetes", "Unknown", "Microbotryomycetes", "Pezizomycetes","Sordariomycetes", "Tremellomycetes", "Mortierellomycetes", "Lecanoromycetes","<1% abund."))
```
...
```{r}
data_glom.fun$Near_Plant = factor(data_glom.fun$Near_Plant, levels = c("Near_Plant", "Not_Near"))
```
...
taxcom_layer_pl = ggplot(data = data_glom.fun, mapping = aes_string(x = "Near_Plant" ,y = "Abundance", fill = "Class" )) + 
  geom_bar(stat="identity", position="fill") + 
  ggtitle("Fungal Taxonomic Composition (Class level) by plant proximity")+
...
```{r}
png("./Figures/Fungal_Taxonomic_Composition_(Class_level)_by_Near_Plant.png", units="in", width = 5.8, height = 5.8, res = 300 )
taxcom_layer_pl
dev.off()
```
