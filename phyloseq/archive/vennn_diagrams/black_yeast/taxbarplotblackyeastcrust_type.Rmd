...
```{r}
data_glom.fun$Genus <- factor(data_glom.fun$Genus, levels = c("Preussia", "Unknown", "Coniosporium", "Cladophialophora", "Neophaeococcomyces", "Leptosphaerulina", "Alternaria", "Macroventuria", "Petrakia", "Corynespora", "<1% abund."))
```
...
```{r}
data_glom.fun$Crust_type = factor(data_glom.fun$Crust_type, levels = c("SMC", "SMC.ster", "SMC.Sub", "Dirt"))
```
...
taxcom_layer_pl = ggplot(data = data_glom.fun, mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Genus" )) + 
  geom_bar(stat="identity", position="fill") + 
  ggtitle("Black Yeast Taxonomic Composition (Genus level) by Crust type")+
...
```{r}
png("./Figures/Black_yeast_Taxonomic_Composition_(Genus_level)_by_Crust_type_reorder_not_Rainbow.png", units="in", width = 5.8, height = 5.8, res = 300 )
taxcom_layer_pl
dev.off()
```
