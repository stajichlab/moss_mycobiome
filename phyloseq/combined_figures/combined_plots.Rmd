...
f <- ggvenn_pq(physeq.prune.rarefy, fact = "Climate", label_alpha = .0, category.names = c("Coastal", "Colorado\nDesert", "Mojave Desert")) +
  ggplot2::scale_fill_distiller(palette = "BuPu", direction = 1)

d <- ggvenn_pq(physeq.prune.rarefy, fact = "Substrate", label_alpha = 0.0, category.names = c("Bare\nSoil", "Subsurface", "Surface Sterile", "Moss\nCrust")) +
  ggplot2::scale_fill_distiller(palette = "BuPu", direction = 1)
...
physeq.prune.rarefy.plot.richness.Substrate = plot_richness(physeq.prune.rarefy, x="Site", color=("Site"), measures=c("Observed")) + geom_boxplot() + stat_compare_means(method = "anova", label.y = 25) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "TP2", label.y = 21, hide.ns = TRUE, size = 7) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_color_manual(values = friendly_pal("ito_seven")) + theme(legend.position="none") 
...
multiplot <- f + physeq.prune.rarefy.plot.richness.Substrate + d + tax_Substrate_class_rhodo_abundance
...
```{r}
pdf("./fish.pdf", width = 16, height = 20)
fish
dev.off()
```